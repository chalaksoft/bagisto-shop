...
| `MARKETPLACE_TOKEN` | — | توکن لایسنس؛ اختیاری، اگر از پنل ثبت‌نام کنید لازم نیست |
...
صفحهٔ «مخزن» تا وقتی لایسنس معتبر نشده، فرم **ثبت‌نام فروشگاه** را نشان می‌دهد:
نام و ایمیل می‌گیرد، به `POST /api/marketplace/register` مخزن می‌فرستد و توکنِ
برگشتی را در جدول `repository_credentials` می‌گذارد. یعنی ادمینی که SSH ندارد
هم می‌تواند لایسنس بگیرد؛ ویرایش `.env` لازم نیست.

لایسنس مقید به دامنه‌ای است که از `APP_URL` خوانده می‌شود، پس **اگر آدرس سایت
عوض شد** باید دامنهٔ لایسنس در مخزن هم به‌روز شود.

توکنِ ثبت‌نام بر `MARKETPLACE_TOKEN` مقدم است: آن کلید معمولاً از یک نصب دیگر
کپی شده و ممکن است روی مخزن اصلاً وجود نداشته باشد؛ اگر مقدم می‌شد، ثبت‌نام
موفق هم چیزی را درست نمی‌کرد.

هر دامنه فقط یک بار ثبت‌نام می‌شود. توکن در مخزن هش‌شده ذخیره می‌شود و قابل
بازیابی نیست، پس اگر با «پاک‌کردن توکن» دورش انداختید، توکن تازه فقط از پنل
مخزن (لایسنس‌ها → صدور توکن تازه) گرفته می‌شود — وگرنه هر کسی که دامنهٔ یک
مشتری را می‌دانست می‌توانست لایسنسش را از دستش دربیاورد.
